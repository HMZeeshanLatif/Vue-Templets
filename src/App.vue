...
console.log(arr.value["products"][0].id);
</script>

<template>
  <div>
    <h1 v-for="(product, index) in arr" :key="index">
      <div v-for="(items, idx) in product" :key="idx">
        <img :src="items.thumbnail" alt="" />
        <h1>{{ items.id }}</h1>
        <h1>{{ items.brand }}</h1>
        <!-- <h1>{{ items.images }}</h1> -->
        <div v-for="(imgs, idnx) in items.images" :key="idnx">
          <img :src="imgs" alt="" />
        </div>
      </div>
    </h1>
